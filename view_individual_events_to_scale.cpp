...
void view_individual_events_to_scale() {
	/*** 
	...
	***/
	...
	TCanvas *c1 = new TCanvas("c1","3D Occupancy for Specified SM Event", 1000, 10, 1700, 1000);
	// c1->SetRightMargin(0.25);
	TPad *pad1 = new TPad("pad1", "pad1", 0.5, 0, 1, 1.0); // left pad
	// pad1->SetRightMargin(0.25); 
	TPad *pad2 = new TPad("pad2", "pad2", 0, 0, 0.5, 0.5); // right pad
	...
	while (readerEventsCR->Next() && !quit) {
		...
		graph_3d->GetXaxis()->SetLimits(0, 130); // ROOT is buggy, x and y use setlimits() // was (0, 20)
		graph_3d->GetYaxis()->SetLimits(-16.8, 115); // but z uses setrangeuser() // was (-16.8, 0)
		...
		graph_avg_d2_per_length->GetYaxis()->SetRangeUser(0, 3); // @@@ change back to 10
		...
		// Ask for input
		if (*duration > 100) { // won't show drawings or ask for input unless its a good event // CHOOSE THIS to show all events or only good events
